...
if [ ! -f "WallboxCtrl/build/wallbox_control_api" ]; then
    echo "❌ wallbox_control_api not found. Building..."
...
echo "2️⃣  Starting API server on port 8080..."
cd WallboxCtrl/build
./wallbox_control_api > /tmp/wallbox_api.log 2>&1 &
PID_API=$!
cd ../..
sleep 2
