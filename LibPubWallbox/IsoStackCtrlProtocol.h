...
namespace Iso15118
{
    ...
    enum class enIsoChargingState : uint8_t
    {
        ...
        identification = 3, ///< The stack awaits an identification confirmation.
        ready = 4,          ///< The charging session is set up but the vehicle not yet requested power transfer by the pilot pin.
        charging = 5,       ///< Power is being active (but may also be zero).
        stop = 6,           ///< Power transfer is interrupted and the charging session is being to be finisched.
        finished = 7,       ///< Charging has been shut down, but the plug is still connected.
        error = 8           ///< An probably resettable error has been occurred. The plug has to be disconnected to reset the error.
    };
...
#pragma pack(push, 1)
    ...
    :------:|:----------:|:---------------------------:|:------:|:--------------:|:-------------:|:--:
    content | msgVersion | msgType                     | enable | identification | currentDemand | -
    unit    | 0          | Iso15118::enIsoStackMsgType | bool   | bool           | A / 10        | -
    ...
    */
    struct stIsoStackCmd
    {
        ...
        uint8_t identification;    ///< Confirmation of a successful identification to continue the session set up.
